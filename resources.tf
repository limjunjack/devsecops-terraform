#images
	...
	name = "chukmunlee/bgg-database:${var.databse_version}"
}
...
resource "docker_container" "bgg-database"{
	...
	volumes {
		colume_name = docker_volume.data-vol.name
		contianer_path = "/var/lib/mysql"
	}
...
output nginx_ip{
	value = digitalocean_droplet.nginx.ipv4
}
